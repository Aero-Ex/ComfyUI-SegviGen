...
try:
    ...
    FAST_SIMPLIFY_AVAILABLE = True
except ImportError:
    print("[SegviGen] Warning: fast-simplification NOT found. Falling back to trimesh simplification.")
    FAST_SIMPLIFY_AVAILABLE = False
...
def build_simplified_work_glb(
    ...
    try:
        if FAST_SIMPLIFY_AVAILABLE:
            # fast_simplification uses reduction ratio (0.0 to 1.0)
            # trimesh uses target face count.
            # Convert target_faces to a ratio if possible, or just use a fixed 0.7 for now
            # as it's common for 'pre-simplification' to be aggressive.
            reduction = max(0.1, 1.0 - (target_faces / src_faces))
            print(f"[Simplify] Using fast-simplification (reduction={reduction:.2f})")
            new_v, new_f = fast_simplification.simplify(mesh.vertices, mesh.faces, target_reduction=reduction)
            simplified = trimesh.Trimesh(vertices=new_v, faces=new_f, process=False)
        else:
            print(f"[Simplify] Using trimesh quadric decimation (target={target_faces})")
        ...
        if simplified is None or len(simplified.faces) == 0:
            raise RuntimeError("Simplification returned empty mesh")
